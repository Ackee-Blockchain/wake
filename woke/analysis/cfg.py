...
class ControlFlowGraph:
    ...
    def __init__(self, declaration: Union[FunctionDefinition, ModifierDefinition]):
        ...
        last = CfgBlock.from_statement(self.__graph, self.__start_block, self.__end_block, None, None, declaration.body)
        self.__graph.add_edge(last, self.__end_block, condition=(TransitionCondition.ALWAYS, None))

        while _normalize(self.__graph, self.__start_block, self.__end_block):
            pass

        if len(self.__end_block.statements) == 0 and len(self.__graph.in_edges(self.__end_block)) == 1:
        ...
        self.__statements_lookup = {stmt: block for block in self.__graph.nodes for stmt in block.statements}
...
def _normalize(graph: nx.DiGraph, start: CfgBlock, end: CfgBlock) -> bool:
    ...
    for node in graph.nodes:
        ...
        if len(node.statements) == 0 and len(graph.out_edges(node)) == 1 and node != start:
        ...
        if len(node.statements) == 0 and len(graph.in_edges(node)) == 0 and node not in {start, end}:
            to_be_removed.add(node)
            changed = True
...
class CfgBlock:
    ...
    __control_statement: Optional[Union[DoWhileStatement, ForStatement, IfStatement, TryStatement, WhileStatement]]
    ...
    def __str__(self):
        return "\n".join(statement.source for statement in self.statements) if len(self.statements) > 0 else ""
    ...
    @property
    def control_statement(self) -> Optional[Union[DoWhileStatement, ForStatement, IfStatement, TryStatement, WhileStatement]]:
    ...
    def from_statement(
        ...
        if isinstance(statement, (Block, UncheckedBlock)):
            for body_statement in statement.statements:
                prev = cls.from_statement(graph, prev, function_end, loop_body, loop_body_next, body_statement)
            return prev
            ...
            graph.add_node(next)
            graph.add_edge(prev, loop_body_next, condition=(TransitionCondition.ALWAYS, None))
            ...
            graph.add_node(next)
            graph.add_edge(prev, loop_body, condition=(TransitionCondition.ALWAYS, None))
            graph.add_edge(prev, next, condition=(TransitionCondition.NEVER, None))
            return next
            ...
            return cls.from_if_statement(graph, prev, function_end, loop_body, loop_body_next, statement)
        elif isinstance(statement, (Return, RevertStatement)):
            prev.__statements.append(statement)
            next = CfgBlock()
            graph.add_node(next)
            graph.add_edge(prev, function_end, condition=(TransitionCondition.ALWAYS, None))
            graph.add_edge(prev, next, condition=(TransitionCondition.NEVER, None))
            return next
        elif isinstance(statement, ExpressionStatement):
            expr = statement.expression
            if isinstance(expr, FunctionCall) and expr.function_called == GlobalSymbolsEnum.REVERT:
                prev.__statements.append(statement)
                next = CfgBlock()
                graph.add_node(next)
                graph.add_edge(prev, function_end, condition=(TransitionCondition.ALWAYS, None))
            ...
            return cls.from_try_statement(graph, prev, function_end, loop_body, loop_body_next, statement)
    ...
    def from_if_statement(
        ...
        true_block_end = cls.from_statement(graph, true_block, function_end, loop_body, loop_body_next, if_statement.true_body)
        ...
        if if_statement.false_body is None:
            ...
            false_block_end = cls.from_statement(graph, false_block, function_end, loop_body, loop_body_next, if_statement.false_body)

        next = CfgBlock()
        graph.add_node(next)
        graph.add_edge(prev, true_block, condition=(TransitionCondition.IS_TRUE, if_statement.condition))
        graph.add_edge(prev, false_block, condition=(TransitionCondition.IS_FALSE, if_statement.condition))
        graph.add_edge(true_block_end, next, condition=(TransitionCondition.ALWAYS, None))
        graph.add_edge(false_block_end, next, condition=(TransitionCondition.ALWAYS, None))
        return next
    ...
    def from_do_while_statement(
        ...
        next = CfgBlock()
        graph.add_node(next)
        body_end = cls.from_statement(graph, body, function_end, body, next, do_while_statement.body)
        ...
        graph.add_edge(body_end, next, condition=(TransitionCondition.IS_FALSE, do_while_statement.condition))
        graph.add_edge(body_end, body, condition=(TransitionCondition.IS_TRUE, do_while_statement.condition))
        return next
    ...
    def from_for_statement(
        ...
        tmp = cls.from_statement(graph, body, function_end, body_end, next, for_statement.body)
        ...
        graph.add_edge(prev, body, condition=(TransitionCondition.IS_TRUE, for_statement.condition))
        graph.add_edge(prev, next, condition=(TransitionCondition.IS_FALSE, for_statement.condition))
        graph.add_edge(body_end, body, condition=(TransitionCondition.IS_TRUE, for_statement.condition))
        graph.add_edge(body_end, next, condition=(TransitionCondition.IS_FALSE, for_statement.condition))
        return next
    ...
    def from_try_statement(
        ...
        success_block_end = cls.from_statement(graph, success_block, function_end, loop_body, loop_body_next, try_statement.clauses[0].block)
        ...
        for clause in try_statement.clauses[1:]:
            if clause.error_name == "Error":
                ...
                revert_block_end = cls.from_statement(graph, revert_block, function_end, loop_body, loop_body_next, clause.block)
            elif clause.error_name == "Panic":
                panic_block = CfgBlock()
                graph.add_node(panic_block)
                panic_block_end = cls.from_statement(graph, panic_block, function_end, loop_body, loop_body_next, clause.block)
            elif clause.error_name == "":
                fail_block = CfgBlock()
                graph.add_node(fail_block)
                fail_block_end = cls.from_statement(graph, fail_block, function_end, loop_body, loop_body_next, clause.block)
            else:
        ...
        graph.add_edge(prev, success_block, condition=(TransitionCondition.TRY_SUCCEEDED, try_statement.external_call))
        graph.add_edge(success_block_end, next, condition=(TransitionCondition.ALWAYS, None))
        if revert_block is not None:
            graph.add_edge(prev, revert_block, condition=(TransitionCondition.TRY_REVERTED, try_statement.external_call))
            graph.add_edge(revert_block_end, next, condition=(TransitionCondition.ALWAYS, None))
        if panic_block is not None:
            graph.add_edge(prev, panic_block, condition=(TransitionCondition.TRY_PANICKED, try_statement.external_call))
            graph.add_edge(panic_block_end, next, condition=(TransitionCondition.ALWAYS, None))
        if fail_block is not None:
            graph.add_edge(prev, fail_block, condition=(TransitionCondition.TRY_FAILED, try_statement.external_call))
            graph.add_edge(fail_block_end, next, condition=(TransitionCondition.ALWAYS, None))
        return next
    ...
    def from_while_statement(
        ...
        next = CfgBlock()
        graph.add_node(next)
        body_end = cls.from_statement(graph, body, function_end, body, next, while_statement.body)

        graph.add_edge(prev, body, condition=(TransitionCondition.IS_TRUE, while_statement.condition))
        graph.add_edge(prev, next, condition=(TransitionCondition.IS_FALSE, while_statement.condition))
        graph.add_edge(body_end, body, condition=(TransitionCondition.IS_TRUE, while_statement.condition))
        graph.add_edge(body_end, next, condition=(TransitionCondition.IS_FALSE, while_statement.condition))
        return next
