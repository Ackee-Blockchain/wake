...
class ModifierDefinition(DeclarationAbc):
    ...
    def implemented(self) -> bool:
        """
        Returns:
            `True` if the modifier [body][woke.ir.declaration.modifier_definition.ModifierDefinition.body] is not `None`, `False` otherwise.
        """
    ...
    def child_modifiers(self) -> FrozenSet[ModifierDefinition]:
        """
        Returns:
            Modifiers that list this modifier in their [base_modifiers][woke.ir.declaration.modifier_definition.ModifierDefinition.base_modifiers] property.
        """
    ...
    def overrides(self) -> Optional[OverrideSpecifier]:
        ...
        !!! example
            ...
            `C.mod` on lines 18-20 has an override specifier with the [overrides][woke.ir.meta.override_specifier.OverrideSpecifier.overrides] property containg two items referencing the contracts `B1` and `B2` ([ContractDefinition][woke.ir.declaration.contract_definition.ContractDefinition]).
            ```solidity linenums="1"
            abstract contract A {
                modifier mod virtual;
            }
