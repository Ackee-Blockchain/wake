...
class VariableDeclaration(DeclarationAbc):
    ...
    !!! example
        ...
        - inside a [ContractDefinition][woke.ir.declaration.contract_definition.ContractDefinition] as a state variable:
            - `:::solidity uint public stateVar` in line 4,
        - inside a [ParameterList][woke.ir.meta.parameter_list.ParameterList]:
            - in an [ErrorDefinition][woke.ir.declaration.error_definition.ErrorDefinition] parameters:
                - `:::solidity uint errorArg` in line 5,
            - in an [EventDefinition][woke.ir.declaration.event_definition.EventDefinition] parameters:
                - `:::solidity uint indexed eventArg` in line 6,
            - in a [FunctionDefinition][woke.ir.declaration.function_definition.FunctionDefinition] parameters or return parameters:
            ...
            - in a [ModifierDefinition][woke.ir.declaration.modifier_definition.ModifierDefinition] parameters:
                - `:::solidity uint modifierArg` in line 12,
            - in a [FunctionTypeName][woke.ir.type_name.function_type_name.FunctionTypeName] parameters or return parameters:
        ...
        - inside a [StructDefinition][woke.ir.declaration.struct_definition.StructDefinition] as a member variable:
            - `:::solidity uint structMember` in line 9,
        - inside a [VariableDeclarationStatement][woke.ir.statement.variable_declaration_statement.VariableDeclarationStatement] in a [FunctionDefinition.body][woke.ir.declaration.function_definition.FunctionDefinition.body] as a local variable:
    ...
    def overrides(self) -> Optional[OverrideSpecifier]:
        ...
        !!! example
            ...
            `B.foo` in line 14 has an override specifier with the [overrides][woke.ir.meta.override_specifier.OverrideSpecifier.overrides] property containing two items referencing the contracts `A1` and `A2` ([ContractDefinition][woke.ir.declaration.contract_definition.ContractDefinition]).
            ```solidity linenums="1"
            contract A1 {
                function foo(uint, bool) external virtual returns(uint) {
                    return 1;
    ...
    def value(self) -> Optional[ExpressionAbc]:
        """
        Is not set if the parent is a [VariableDeclarationStatement][woke.ir.statement.variable_declaration_statement.VariableDeclarationStatement].
        In this case, the initial value (if any) is set in the [VariableDeclarationStatement.initial_value][woke.ir.statement.variable_declaration_statement.VariableDeclarationStatement.initial_value] property.
        Returns:
