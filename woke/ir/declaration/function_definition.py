...
class FunctionDefinition(DeclarationAbc):
    ...
    def implemented(self) -> bool:
        """
        Returns:
            `True` if the function [body][woke.ir.declaration.function_definition.FunctionDefinition.body] is not `None`, `False` otherwise.
        """
    ...
    def child_functions(
        ...
        """
        Returns:
            Functions that list this function in their [base_functions][woke.ir.declaration.function_definition.FunctionDefinition.base_functions] property.
        """
    ...
    def overrides(self) -> Optional[OverrideSpecifier]:
        ...
        !!! example
            ...
            `B.foo` on lines 12-14 has an override specifier with the [overrides][woke.ir.meta.override_specifier.OverrideSpecifier.overrides] property containg one item referencing the contract `A` ([ContractDefinition][woke.ir.declaration.contract_definition.ContractDefinition]).
            ```solidity linenums="1"
            interface I {
                function foo() external returns(uint);
            }
