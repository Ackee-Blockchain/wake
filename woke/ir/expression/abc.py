...
class ExpressionAbc(SolidityAbc, ABC):
    ...
    def type(self) -> Optional[TypeAbc]:
        """
        Can be `None` in case of an [Identifier][woke.ir.expression.identifier.Identifier] in an [ImportDirective][woke.ir.meta.import_directive.ImportDirective].
        !!! example
    ...
    def type_string(self) -> Optional[str]:
        """
        !!! example
            `:::solidity function (uint256,uint256) returns (uint256)` in the case of the `foo` [Identifier][woke.ir.expression.identifier.Identifier] in the `:::solidity foo(1, 2)` expression for the following function:
            ```solidity
    ...
    def is_ref_to_state_variable(self) -> bool:
        """
        In many cases it may be useful to know if an [Assignment][woke.ir.expression.assignment.Assignment] to an expression modifies a state variable or not.
        This may not be straightforward to determine, e.g. if the expression is a [MemberAccess][woke.ir.expression.member_access.MemberAccess] or [IndexAccess][woke.ir.expression.index_access.IndexAccess] to a state variable.
        Returns:
