...
from woke.ir.ast import AstNodeId, ExternalReferenceModel, SolcInlineAssembly
...
from woke.ir.yul.block import YulBlock
...
class ExternalReference:
    ...
    !!! warning
        This is not an IR node, but a helper class for [InlineAssembly][woke.ir.statements.inline_assembly.InlineAssembly].
        Since this is not an IR node, there must still be a Yul IR node ([YulIdentifier][woke.ir.yul.identifier.YulIdentifier]) in the source code that represents the identifier.
    """
...
class InlineAssembly(StatementAbc):
    ...
    def yul_block(self) -> YulBlock:
        """
        Returns:
            Yul block containing Yul IR nodes ([YulAbc][woke.ir.yul.abc.YulAbc]).
        """
