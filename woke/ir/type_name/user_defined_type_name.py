...
class UserDefinedTypeName(TypeNameAbc):
    ...
    In Solidity 0.8.0 a new IR node ([IdentifierPath][woke.ir.meta.identifier_path.IdentifierPath]) was introduced to replace [UserDefinedTypeName][woke.ir.type_name.user_defined_type_name.UserDefinedTypeName] in some cases.
    ...
    def name(self) -> str:
        """
        !!! note
            Should be the same as [source][woke.ir.abc.IrAbc.source] and is the same as [path_node.name][woke.ir.meta.identifier_path.IdentifierPath.name] if [path_node][woke.ir.type_name.user_defined_type_name.UserDefinedTypeName.path_node] is not `None`.
        Returns:
