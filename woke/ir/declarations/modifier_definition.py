...
from woke.ir.ast import AstNodeId, SolcModifierDefinition, SolcStructuredDocumentation
from woke.ir.enums import Visibility
...
class ModifierDefinition(DeclarationAbc):
    ...
    def implemented(self) -> bool:
        """
        Returns:
            `True` if the modifier [body][woke.ir.declarations.modifier_definition.ModifierDefinition.body] is not `None`, `False` otherwise.
        """
    ...
    def child_modifiers(self) -> FrozenSet[ModifierDefinition]:
        """
        Returns:
            Modifiers that list this modifier in their [base_modifiers][woke.ir.declarations.modifier_definition.ModifierDefinition.base_modifiers] property.
        """
    ...
    def overrides(self) -> Optional[OverrideSpecifier]:
        ...
        !!! example
            ...
            `B1.mod` on lines 6-8 has an override specifier with the [overrides][woke.ir.meta.override_specifier.OverrideSpecifier.overrides] property empty.

            `B2.mod` on lines 12-14 has an override specifier with the [overrides][woke.ir.meta.override_specifier.OverrideSpecifier.overrides] property empty.

            `C.mod` on lines 18-20 has an override specifier with the [overrides][woke.ir.meta.override_specifier.OverrideSpecifier.overrides] property containg two items referencing the contracts `B1` and `B2` ([ContractDefinition][woke.ir.declarations.contract_definition.ContractDefinition]).
            ```solidity linenums="1"
            abstract contract A {
                modifier mod virtual;
            }
