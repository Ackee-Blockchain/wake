...
from woke.ir.enums import DataLocation, Mutability, Visibility
from woke.ir.expressions.abc import ExpressionAbc
from woke.ir.meta.structured_documentation import StructuredDocumentation
...
class VariableDeclaration(DeclarationAbc):
    ...
    !!! example
        ...
        - inside a [ContractDefinition][woke.ir.declarations.contract_definition.ContractDefinition] as a state variable:
            - `:::solidity uint public stateVar` in line 4,
        - inside a [ParameterList][woke.ir.meta.parameter_list.ParameterList]:
            - in an [ErrorDefinition][woke.ir.declarations.error_definition.ErrorDefinition] parameters:
                - `:::solidity uint errorArg` in line 5,
            - in an [EventDefinition][woke.ir.declarations.event_definition.EventDefinition] parameters:
                - `:::solidity uint indexed eventArg` in line 6,
            - in a [FunctionDefinition][woke.ir.declarations.function_definition.FunctionDefinition] parameters or return parameters:
            ...
            - in a [ModifierDefinition][woke.ir.declarations.modifier_definition.ModifierDefinition] parameters:
                - `:::solidity uint modifierArg` in line 12,
            - in a [FunctionTypeName][woke.ir.type_names.function_type_name.FunctionTypeName] parameters or return parameters:
        ...
        - inside a [SourceUnit][woke.ir.meta.source_unit.SourceUnit] only as a constant variable:
            - `:::solidity uint constant CONST = 10` in line 1,
        - inside a [StructDefinition][woke.ir.declarations.struct_definition.StructDefinition] as a member variable:
            - `:::solidity uint structMember` in line 9,
        - inside a [VariableDeclarationStatement][woke.ir.statements.variable_declaration_statement.VariableDeclarationStatement] in a [FunctionDefinition.body][woke.ir.declarations.function_definition.FunctionDefinition.body] as a local variable:
    ...
    def data_location(self) -> DataLocation:
        """
        [DataLocation.DEFAULT][woke.ir.enums.DataLocation.DEFAULT] is returned if the data location is not specified in the source code.
        Returns:
    ...
    def overrides(self) -> Optional[OverrideSpecifier]:
        ...
        !!! example
            ...
            `B.foo` in line 14 has an override specifier with the [overrides][woke.ir.meta.override_specifier.OverrideSpecifier.overrides] property containing two items referencing the contracts `A1` and `A2` ([ContractDefinition][woke.ir.declarations.contract_definition.ContractDefinition]).
            ```solidity linenums="1"
            contract A1 {
                function foo(uint, bool) external virtual returns(uint) {
                    return 1;
    ...
    def value(self) -> Optional[ExpressionAbc]:
        """
        Is not set if the parent is a [VariableDeclarationStatement][woke.ir.statements.variable_declaration_statement.VariableDeclarationStatement].
        In this case, the initial value (if any) is set in the [VariableDeclarationStatement.initial_value][woke.ir.statements.variable_declaration_statement.VariableDeclarationStatement.initial_value] property.
        Returns:
