...
- Fuzzing flows now expect `str | None` as the return value
- `None` is interpreted as a successful execution
- Any string is interpreted as unsuccessful with a given (string) reason
- Only successful executions are accounted for in the `max_times` flow decorator parameter limit

**Statistics display:**

- Fuzzing stats are automatically printed at the end of the fuzzing session
- Use `-v` flag for better visualization of execution counts for each flow with all collected statuses
